...
当前版本已经实现纯对话单会话 TUI：可以在终端启动、读取配置、连接 Anthropic 或 OpenAI-compatible 流式 Provider、进行多轮上下文对话，并通过 tmux/psmux E2E 验证真实 CLI 行为。
...
这些能力会在后续设计 tool execution、permission layer 和 session persistence 后再拆分实现。
...
- 检查流式输出、多轮上下文和 sentinel API key 不泄露。
