...
| 组件 | 职责 | 状态性 |
|------|------|--------|
| PermissionChecker | 编排管道各层，返回最终判定 | 有状态（持有可变 session rules + mode） |
| BlacklistGuard | 正则匹配危险命令 | 无状态 |
| PathSandbox | 委托 workspace.ts 检查路径边界 | 无状态 |
| RuleEngine | 编译和匹配三层规则 | 无状态（CompiledRule 持有预编译 matcher） |
| AutoSafetyRules | 硬编码安全白名单匹配 | 无状态 |
| ModePolicy | 按模式返回默认判定 | 无状态 |
| PermissionConfig | 加载/合并三层 YAML 规则 | 一次性加载后不可变 |
| PermissionPromptCoordinator | FIFO 串行权限请求、超时和 exactly-once settlement | 有状态（TUI 外部 store） |
| AskPermissionFn | TUI 弹窗回调（注入） | 由 coordinator 提供稳定函数 |
...
export interface CompiledRule {
  ...
  matcher: ((target: string) => boolean) | undefined; // 规则加载/新增时编译一次
  action: 'allow' | 'deny';
}
...
**匹配顺序：** session → project → global，每层内按声明顺序从上到下，first-match-wins。动态 session 规则加入同一个 session 数组头部，不再经过独立 allowlist。`*` 不跨分隔符，`**` 跨分隔符，`\*` 匹配字面星号；matcher 在加载/新增规则时编译一次，检查热路径只执行函数。未命中返回 `undefined`。
...
- `auto` → `'needs_prompt'`
- `yolo` → allow
- `plan` → read allow；write/execute deny（同时保留注册层过滤）

### 可变 Session 规则层

**职责：** 在 PermissionChecker 内持有会话级 `CompiledRule[]`，作为 RuleEngine 的最高优先级层。
**行为：** 初始化时复制配置的 session 规则；运行时授权统一通过 `compileRule()` 构建，并 `unshift()` 到数组头部。这样动态 session grant 能覆盖 project/global 规则，同时完全复用 RuleEngine 的目标提取、matcher 与 first-match-wins 语义。
...
3. plan + write/execute → 防御性 deny（不可被 allow 规则绕过）
4. `matchRules(input, ruleConfig)` → 按 session/project/global 命中 allow/deny
5. `checkAutoSafety(input, mode)` → 命中则 allow
6. `applyModeDefault(input, mode)` → strict→deny, normal/auto→needs_prompt, yolo→allow, plan read→allow
7. 若 needs_prompt：
   - 有 `askFn` → 调用并等待（30s 超时，超时 deny）
   - 无 `askFn` → deny（fail-safe）
8. 处理 askFn 响应：
   - `allow_once` → return allowed
   - `allow_session` → 生成 CompiledRule 加入最高优先级 session 数组头部 → return allowed
...
**关键设计点：** TUI 弹窗阻塞的是 `executeToolCall` 的 async promise。PermissionPromptCoordinator 对可能并发到达的请求使用 FIFO 串行展示，仅活动请求启动 30 秒计时，并在响应、超时或卸载时 exactly-once settle。Ink 事件循环不受阻塞影响，可持续渲染 UI 和接收按键。
...
src/tui/
├── permissionPromptCoordinator.ts  — FIFO 权限请求协调器（新建）
└── components/PermissionPrompt.tsx — F5: Ink 阻塞式弹窗组件（新建）
...
├── autoSafety.test.ts
├── modePolicy.test.ts
├── PermissionPromptCoordinator.test.ts
├── config.test.ts
├── checker.test.ts
└── prompt-flow.test.ts
```
