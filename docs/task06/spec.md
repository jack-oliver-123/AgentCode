...
| 模式 | 未命中时行为 | 说明 |
|------|-------------|------|
| plan | write/execute 工具不可用 | 复用现有 filterByRisk 机制 |
| strict | 直接拒绝 | 只有显式 allow 规则命中的操作才能执行 |
| normal（默认） | 弹窗确认 | 日常开发模式 |
| auto | 内置安全规则判断 | 已知安全自动放行，不确定弹窗 |
| yolo | 自动允许 | 完全信任，快速迭代 |

权限模式通过项目配置文件 `.agentcode/config.yaml` 的 `permission_mode` 字段设置，默认值为 `normal`。运行时可通过 TUI 快捷键切换（复用现有 Tab 切换 plan/full 的模式切换机制）。
...
当权限判定结果为"需要确认"时，以阻塞式 TUI 弹窗呈现工具调用摘要，Agent Loop 暂停等待用户选择后才继续执行。
...
plan 模式在工具注册阶段已通过 filterByRisk 过滤，不经过此管道。

## 非功能需求

- **N1：性能** — 权限判定管道（不含人在回路等待）的开销不超过 5ms。规则文件在会话启动时一次性加载到内存，运行时不读文件。
...
- plan 模式复用现有 filterByRisk，权限系统不重新实现工具过滤
...
- **AC5：五档模式行为** — 对一个未被任何规则命中的工具调用：
  - plan 模式：write/execute 工具不出现在模型可用工具列表中
