...
describe('run_command', () => {
  it('runs a successful one-shot command in the workspace', async () => {
    ...
    const result = await executeRunCommand(JSON.stringify({ command: nodeCommand('process.stdout.write(process.cwd())') }), {
      cwd: workspace
    });
  ...
  it('rejects invalid argument shapes before spawning a command', async () => {
    ...
    const result = await executeFileTool(createRunCommandTool(), '{"command":"node --version","timeoutMs":0}', { cwd: workspace });
  ...
  it('does not read non-interactive bash startup files from BASH_ENV', async () => {
    ...
    const bashEnvPath = await writeWorkspaceFile(workspace, 'bash-env.sh', `echo bash-env-loaded > ${JSON.stringify(join(workspace, 'bash-env-loaded.txt'))}`);
  ...
  it('reports shell startup failures as command_failed', async () => {
    ...
    const result = await executeFileTool(createRunCommandTool(), JSON.stringify({ command: nodeCommand("process.stdout.write('never')") }), {
      cwd: invalidCwd
    });
